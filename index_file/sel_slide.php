...
function openFrame(href){
	if(parent.popwin != null)
		parent.popwin.close();
	parent.popwin = window.open (href, lecture+"--"+chapter, "status=yes,location=no,menubar=no,width=900,height=350");
}
...
<div id='info'>
<ul>
<li>Length: <p><?php echo substr(@$ch_info['lecture_len'],0,strlen(@$ch_info['lecture_len'])-2);?><p></li>
...
<div class='btn' id='playAll'><a onclick="openFrame('../newform.php?chapter=<?php echo $slide;?>&lecture=DSP');">Play Whole</a></div>
<h5>Key Terms:</h5>
...
for($i = 0; !empty($keywords[$i]); $i++){
	...
	echo "<a onmouseover='show_hint_msg(this,1);' onmouseout='hide_hint_msg();' onclick='showkeyterm(\"$keywords[$i]\")' name='".str_replace(' ', '_', $keywords[$i])."'>".$keywords[$i]."</a>";
	echo '</div>';
	echo "<div class='keyword'>\n";
	for($j = 0; !empty($keyterms[$j]);$j++){
		echo "<a onmouseover='show_hint_msg(this,2);' onmouseout='hide_hint_msg();' href='#".str_replace(' ', '_', $keyterms[$j])."' onclick='showkeyterm(\"".$keyterms[$j]."\")'>".$keyterms[$j]."</a>";
		if($j > 3) break;
	}
	echo "</div></div>\n";
}
...
<div id="hint_msg" style="width:300px;height:120px;position:absolute;top:510px;left:280px;display:none;border: solid purple 5px;background-color:pink;">
</div>
</body>
</html>
...
function getIE(e){
	...
	t+=30;
	l+=20;
	document.getElementById("hint_msg").style.top=t+'px';
	document.getElementById("hint_msg").style.left=l+'px';
}
function show_hint_msg(obj,num){
	//getIE(obj);
	document.getElementById("hint_msg").style.display='block';
	if(num==1)
		document.getElementById("hint_msg").innerHTML='Only those key terms in black on the top of each column are the key terms appearing in this slide';
	else
		document.getElementById("hint_msg").innerHTML='In each column the key terms in blue are those appearing in the course but losely related to the key term of this slide in black on the top of the column';
}
function hide_hint_msg(){
	document.getElementById("hint_msg").style.display='none';
}
</script>
