...
for($i = 0; !empty($keywords[$i]); $i++){
	...
	echo "<a onmouseover='show_hint_msg(this,1);' onmouseout='hide_hint_msg();' onclick='showkeyterm(\"".str_replace("'", "\'",$keywords[$i])."\")' name='".str_replace(' ', '_', $keywords[$i])."'>".$keywords[$i]."</a>";
	echo '</div>';
	echo "<div class='keyword'>\n";
	for($j = 0; !empty($keyterms[$j]);$j++){
		echo "<a onmouseover='show_hint_msg(this,2);' onmouseout='hide_hint_msg();' href='#".str_replace(' ', '_', trim($keyterms[$j]))."' onclick=\"showkeyterm('".str_replace("'", "\'",$keyterms[$j])."')\">".$keyterms[$j]."</a>";
		if($j > 3) break;
	}
	echo "</div></div>\n";
}
...
<div id="hint_msg" style="width:300px;height:120px;position:absolute;top:470px;left:280px;display:none;border: solid purple 5px;background-color:pink;">
</div>
...
function show_hint_msg(obj,num){
	...
	if(num==1)
		document.getElementById("hint_msg").innerHTML='Only those key terms in black on the top of each column are the key terms appearing in this chapter';
	else
		document.getElementById("hint_msg").innerHTML='In each column the key terms in blue are those appearing in the course but losely related to the key term of this chapter in black on the top of the column';
}
